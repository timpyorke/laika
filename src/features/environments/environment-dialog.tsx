...
const selectClass = "h-8 cursor-pointer rounded-md border border-[var(--border)] bg-[var(--background)] px-2.5 text-[12.5px]";
...
function VariableRow({ variable }: { variable: EnvironmentVariable }) {
  ...
  return (
    <div className="grid grid-cols-[minmax(110px,0.8fr)_minmax(150px,1.2fr)_auto] items-center gap-2 border-b border-[var(--border-subtle)] py-2">
      ...
      <div className="flex items-center gap-1">
        <label className="flex items-center gap-1 text-[11.5px] text-[var(--muted)]"><input type="checkbox" checked={secret} onChange={(event) => { setSecret(event.target.checked); setValue(""); setRevealed(null); }} /> Secret</label>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-h-[85vh] w-[min(760px,calc(100vw-32px))] overflow-auto panel-scroll" title="Environments & secrets" description="Workspace values apply everywhere; the active environment overrides matching names.">
        <section className="rounded-md border border-[var(--border)] bg-[var(--surface-sunken)] p-3">
          <div className="font-display flex items-center gap-2 text-[13.5px] font-semibold">{vault.unlocked ? <UnlockKeyhole size={16} /> : <LockKeyhole size={16} />} Secret vault</div>
          {vault.unlocked ? (
            <div className="mt-2 flex items-center justify-between text-[11.5px] text-[var(--muted)]"><span>Unlocked. Secret values require an explicit reveal, copy, save, or send action.</span><Button variant="secondary" size="sm" onClick={() => void lock()}>Lock</Button></div>
          ) : (
        ...
        <section className="mt-5 grid gap-3">
          <label className="grid gap-1 text-[12.5px] font-medium">Active environment
        ...
        <section className="mt-5">
          <div className="flex items-end justify-between gap-3">
            <div><h3 className="font-display text-[13.5px] font-semibold">Variables</h3><p className="mt-1 text-[11.5px] text-[var(--muted)]">Reference values with <code>{"{{name}}"}</code>.</p></div>
            <label className="grid gap-1 text-[11.5px] text-[var(--muted)]">Scope<select className={selectClass} value={scope} onChange={(event) => setScope(event.target.value)}><option value="workspace">Workspace</option>{environments.map((environment) => <option key={environment.id} value={environment.id}>{environment.name}</option>)}</select></label>
          </div>
          <form className="mt-3 grid grid-cols-[minmax(110px,0.8fr)_minmax(150px,1.2fr)_auto] items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--surface-sunken)] p-2" onSubmit={addVariable}>
            ...
            <div className="flex items-center gap-2"><label className="flex items-center gap-1 text-[11.5px]"><input type="checkbox" checked={variableSecret} onChange={(event) => setVariableSecret(event.target.checked)} /> Secret</label><Button type="submit" size="sm" disabled={!variableName.trim() || (variableSecret && !vault.unlocked)}><Plus size={14} /> Add</Button></div>
          </form>
          <div className="mt-2">{scopedVariables.length === 0 ? <p className="py-6 text-center text-[11.5px] text-[var(--muted)]">No variables in this scope.</p> : scopedVariables.map((variable) => <VariableRow key={variable.id} variable={variable} />)}</div>
        </section>
      </DialogContent>
    </Dialog>
  );
