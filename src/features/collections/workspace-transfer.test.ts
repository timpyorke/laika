...
describe("Laika collection export", () => {
  it("preserves non-secret request data while omitting auth secret material", () => {
    const payload = buildWorkspaceExport(
      ...
      [{ id: "r1", collectionId: "c1", folderId: "f1", name: "List", method: "GET", url: "https://example.com", params: [{ enabled: true, key: "page", value: "2" }], headers: [], bodyMode: "none", body: "", form: [], auth: { type: "bearer" }, hasAuthSecret: true, timeoutMs: 30_000, assertions: [{ id: "a1", kind: "status", operator: "equals", target: "", expected: "200" }], extractions: [] }],
    );
    ...
    expect(roundTrip.collections[0].requests[0].assertions[0].expected).toBe("200");
