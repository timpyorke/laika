...
const selectClass = "h-8 w-full rounded-md border border-[var(--border)] bg-[var(--surface)] px-2 text-xs";
...
export function TestRunnerPanel() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="grid gap-2 border-b border-[var(--border)] p-2">
      ...
      {collectionId && requestCount === 0 ? <p className="text-xs text-[var(--muted)]">Add a saved request to this collection before running it.</p> : null}
    </div>
    ...
    <div className="min-h-0 flex-1 overflow-auto p-2 panel-scroll">
      {run ? <section aria-label="Test run results" className="grid gap-2">
        <div className="rounded-md border border-[var(--border)] bg-[var(--background)] p-2">
          <div className="flex items-center gap-2">
            ...
            <strong className="min-w-0 flex-1 truncate text-xs">{run.collectionName}</strong>
        ...
        {run.results.map((result) => <div key={result.id} className="rounded-md border border-[var(--border)] p-2 text-xs">
      ...
      </section> : <div className="py-8 text-center text-xs text-[var(--muted)]">Run a collection to see assertion results.</div>}

      {recent.length ? <section className="mt-4 border-t border-[var(--border)] pt-3" aria-label="Recent test runs"><h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-[var(--muted)]">Recent runs</h3><div className="grid gap-1">{recent.map((item) => <button key={item.id} type="button" className="flex items-center gap-2 rounded p-1.5 text-left text-xs hover:bg-[var(--surface-muted)]" onClick={() => void open(item.id)}>{item.status === "passed" ? <CheckCircle2 className="text-[var(--success)]" size={13} /> : <XCircle className="text-[var(--danger)]" size={13} />}<span className="min-w-0 flex-1 truncate">{item.collectionName}</span><span className="text-[10px] text-[var(--muted)]">{item.passedRequests}/{item.totalRequests}</span></button>)}</div></section> : null}
    </div>
  </div>;
}
