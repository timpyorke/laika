  ...
  return {
    ...
    collections: collections.map((collection) => ({
      ...
      requests: requests.filter((request) => request.collectionId === collection.id).map((summary) => {
        ...
        return { folderSourceId: saved.folderId, name: saved.name, method: saved.method, url: saved.url, params: saved.params, headers: saved.headers, bodyMode: saved.bodyMode, body: saved.body, form: saved.form, auth: saved.auth, authSecret: null, timeoutMs: saved.timeoutMs };
      }),
    })),
  };
}
